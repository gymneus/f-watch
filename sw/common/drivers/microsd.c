...
void MICROSD_Init(void)
{
  ...
  MICROSD_USART->ROUTE = USART_ROUTE_TXPEN | USART_ROUTE_RXPEN |
                         USART_ROUTE_CLKPEN | (MICROSD_LOC << 8);
  ...
  /* IO Configuration for Power-On/Off the card */
  GPIO_PinModeSet(MICROSD_POWER_PORT, MICROSD_POWER_PIN, gpioModePushPull, 0); /* SD_EN controls power on/off */
}
...
void MICROSD_Deinit(void)
{
  ...
  /* IO Configuration for Power-On/Off the card */
  GPIO_PinModeSet(MICROSD_POWER_PORT, MICROSD_POWER_PIN, gpioModeDisabled, 0); /* SD_EN */
}
 ...
 *        The freewatch supports socket power control, enable also the SPI clock.
 *****************************************************************************/
void MICROSD_PowerOn(void)
{
  /* Power-On the microSD */
  uint8_t pins_mask = 1 << MICROSD_POWER_PIN;
  GPIO_PortOutSetVal(MICROSD_POWER_PORT, pins_mask, pins_mask);
 ...
 *        The freewatch supports socket power control, disable also the SPI clock.
 *****************************************************************************/
void MICROSD_PowerOff(void)
{
  /* Power-On the microSD */
  uint8_t pins_mask = 1 << MICROSD_POWER_PIN;
  GPIO_PortOutSetVal(MICROSD_POWER_PORT, pins_mask, 0);
